...
usage() {
    ...
    echo -e "usage: $script_name -s <path to file> -w <prefix whitespace count>"
    ...
    echo -e "\nEXAMPLE:"
    echo "$ $script_name -s ~/somefile.txt -w  5"
}
...
if [[ ! -f "$path_to_file" ]]; then
    echo -e "ERROR: The file does not exist: $path_to_file\n"
    usage
    exit 1
fi
...
if [[ "$disable_grep_ignores" = true ]]; then
    awk -v prefix="$prefix" '{print prefix $0}' "$path_to_file"
else
